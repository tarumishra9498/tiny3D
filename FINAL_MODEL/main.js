...
const viewer_12 = createHeartViewerGLB(document.getElementById('viewer12'),"tof.glb");
viewers.push(viewer_12);

//DUCTUS ARTERIOSIS
const viewer_98 = createHeartViewerGLB(document.getElementById('viewer98'),"duct.glb");
viewers.push(viewer_98);
...
function createSlicerViewer(container, slider, modelPath) {
    ...
    loader.load(modelPath, (gltf) => {
        ...
        // Simple default view (no button-based reorientation)
        camera.position.set(0, 10, 50);
...
function createMultiPlaneOverview(container, modelPath, planeConfigs = []) {
    if (!container) {
        console.error("createMultiPlaneOverview: container not found");
        return null;
    }
    ...
    const spotLight = new THREE.SpotLight(0xffffff, 2.5, 300, 0.25, 0.5);
    spotLight.position.set(30, 40, 30);
    scene.add(spotLight);

    // 🎨 Plane colors (bottom → top)
    const planeColors = [
        0xff9999, // red
        0xffff99, // yellow
        0x99ff99, // green
    ...
    function makePlane(width, height, cfg, index) {
        const geometry = new THREE.PlaneGeometry(width, height);

        const material = new THREE.MeshBasicMaterial({
            color: planeColors[index] || 0xffffff,
            transparent: true,
            opacity: 0.20,
            side: THREE.DoubleSide,
            depthWrite: false
    ...
    loader.load(
        ...
        (gltf) => {
            ...
            model.position.y += 1.65; // adjust this number
           

            const centeredBox = new THREE.Box3().setFromObject(model);
            const centeredSize = centeredBox.getSize(new THREE.Vector3());
            const centeredMaxDim = Math.max(centeredSize.x, centeredSize.y, centeredSize.z);

            const planeWidth = centeredSize.x * 1.1;
            const planeHeight = centeredSize.z * 0.88;

            const fov = THREE.MathUtils.degToRad(camera.fov);
            let distance = (centeredMaxDim / 2) / Math.tan(fov / 2);
            distance *= 1.9;

            camera.position.set(0, centeredMaxDim * 0.02, distance);
            camera.near = 0.1;
            camera.far = Math.max(1000, centeredMaxDim * 10);
            camera.updateProjectionMatrix();
            ...
            const defaults = [
                { y: -centeredSize.y * 0.32 },
                { y: -centeredSize.y * 0.10 },
                { y:  centeredSize.y * 0.12 },
                { y:  centeredSize.y * 0.34 }
            ];

            const configsToUse = planeConfigs.length ? planeConfigs : defaults;

            configsToUse.forEach((cfg, index) => {
                makePlane(planeWidth, planeHeight, cfg, index);
            });

            forceResizeSequence();

            console.log("Bottom overview loaded successfully");
        },
        undefined,
        (error) => {
            console.error("Failed to load model in bottom overview:", error);
        }
    );

    window.addEventListener("resize", forceResizeSequence);
...
/* ---------------- CREATE MULTI PLANE VIEWERS 5---------------- */
const viewbot = createMultiPlaneOverview(
    document.getElementById("viewerBottom"),
    "updatedForamen.glb",
    [
        { y: 0},
        { y: 1.5},
